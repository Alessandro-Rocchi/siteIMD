...
document.addEventListener("DOMContentLoaded", () => {
  fetch('assets/data/interview.json')
    ...
    // Elementi di riferimento del Lightbox unico
    const lightboxModalEl = document.getElementById("lightboxModal");
    
    // Se il modal non è presente in questa specifica pagina HTML, interrompe lo script senza errori
    if (!lightboxModalEl) return;

    // Inizializza l'istanza del Modal di Bootstrap 5 via JS
    const lightboxModal = new bootstrap.Modal(lightboxModalEl);
    const lightboxImage = lightboxModalEl.querySelector(".lightbox-img");

    // Gestore di eventi globale (Event Delegation)
    document.addEventListener("click", (event) => {
      // Controlla se l'elemento cliccato ha la classe delle immagini ingrandibili
      if (event.target.classList.contains("lightbox-trigger")) {
        
        // Preleva il percorso dell'immagine cliccata
        const sorgenteImmagine = event.target.getAttribute("src");
        const testoAlternativo = event.target.getAttribute("alt") || "Ingrandimento immagine";
        
        // Inietta istantaneamente i dati dentro il modal vuoto
        lightboxImage.setAttribute("src", sorgenteImmagine);
        lightboxImage.setAttribute("alt", testoAlternativo);
        
        // Ordina a Bootstrap di mostrare la finestra
        lightboxModal.show();
      }
    });
    const xmindBtn = document.getElementById("loadXmindBtn");
    const xmindPlaceholder = document.getElementById("xmind-placeholder");
    const xmindContainer = document.getElementById("xmind-container");

    if (xmindBtn) {
        xmindBtn.addEventListener("click", () => {
            
            // 1. Nascondi il riquadro finto
            xmindPlaceholder.classList.add("d-none");
            
            // 2. Mostra il contenitore vuoto
            xmindContainer.classList.remove("d-none");
            
            // 3. Inietta l'iframe reale di XMind (ora può rubare il focus quanto vuole, l'utente è già lì!)
            xmindContainer.innerHTML = `
                <iframe 
                    src="https://app.xmind.com/embed/MBf97hPz?sheet-id=57d99461f8896cf0547b8aecf8" 
    ...
    pane.innerHTML = `
      <div class="border-bottom pb-3 mb-4">
        ...
        <div class="d-flex gap-2 flex-wrap">
          ...
          <span class="badge border border-secondary text-secondary">Age: ${persona.age}</span>
        </div>
      </div>
